...
class Solution:
    def lowestCommonAncestor(self, root: 'TreeNode', p: 'TreeNode', q: 'TreeNode') -> 'TreeNode':        
        # BST Traversal | Time: O(N) | Space: O(1)
        # N is the size of nodes in the Binary Tree.
        # In the worst case, we need to traverse all the nodes in BST.
        root_ptr = root
        while root_ptr:
            if p.val < root_ptr.val and q.val < root_ptr.val:
                root_ptr = root_ptr.left
            elif p.val > root_ptr.val and q.val > root_ptr.val:
