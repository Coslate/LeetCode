    ...
    def exist(self, board: List[List[str]], word: str) -> bool:
        ...
        for i in range(row_num):
            for j in range(col_num):
                if visit[i][j] == False:
                    (ans, word_idx) = DFSCall(board, word, i, j, word_idx, got_mask, visit)
                elif visit[i][j] == True and got_mask[i][j] == True:
                    if i+1 < row_num and visit[i+1][j] == False:
                        (ans, word_idx) = DFSCall(board, word, i+1, j, word_idx, got_mask, visit)
                    elif j+1 < col_num and visit[i][j+1] == False:
                        (ans, word_idx) = DFSCall(board, word, i, j+1, word_idx, got_mask, visit)
                    elif i-1 >= 0 and visit[i-1][j] == False:
                        ans = DFSCall(board, word, i-1, j)
                        (ans, word_idx) = DFSCall(board, word, i-1, j, word_idx, got_mask, visit)
